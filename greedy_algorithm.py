...
############################################
    def greedy(self, fire_exit, env):
        ...
        for move in moves:
            ...
            #avoids stacking
            for other_actor in env.actors:
                if other_actor != self and np.linalg.norm(np.array(other_actor.pos) - np.array(new_pos)) < self.radius + other_actor.radius:
                    angle_to_other = np.arctan2(other_actor.pos[1] - new_pos[1], other_actor.pos[0] - new_pos[0])
                    new_pos = [new_pos[0] - np.cos(angle_to_other) * 2, new_pos[1] - np.sin(angle_to_other) * 2]

            if not env.detect_collision(self, new_pos):
                dis = self.manhattan_dis(new_pos, fire_exit)
                if dis < min_dis:
                    min_dis = dis
                    best_move = new_pos
...
class BlueprintEnvironment:
    ...
    def update_actors(self):
        ...
        for actor in self.actors[:]:
            ...
            #making sure the actors don't stack
            for other_actor in self.actors:
                if other_actor != actor and np.linalg.norm(np.array(actor.pos) - np.array(other_actor.pos)) < actor.radius + other_actor.radius:
                    self.collision(actor, other_actor)
    ...
    #avoids collision with other actors
    def collision(self, actor, other_actor):
        direction = np.array(actor.pos) - np.array(other_actor.pos)
        distance = np.linalg.norm(direction)
        if distance < actor.radius + other_actor.radius:
            direction = direction / distance
            actor.pos = np.array(actor.pos) + direction * (actor.radius + other_actor.radius - distance)
    ...
    def update_adult(self, adult):
        ...
        new_pos = adult.greedy(exit_pos, self)
        if not np.array_equal(new_pos, adult.pos):
            adult.pos = new_pos
    ...
    def update_staff(self, staff):
        ...
        if staff.guiding:
            ...
            new_pos = staff.greedy(exit_pos, self)
            staff.previous_pos = staff.pos[:]
            if not self.detect_collision(staff, new_pos):
                staff.pos = new_pos
    ...
    def move_towards(self, actor, target, speed):
        ...
        if np.linalg.norm(direction) > 0:
            ...
            #to avoid stacking
            for other_actor in self.actors:
                if other_actor != actor and np.linalg.norm(np.array(other_actor.pos) - new_pos) < actor.radius + other_actor.radius:
                    angle_to_other = np.arctan2(other_actor.pos[1] - new_pos[1], other_actor.pos[0] - new_pos[0])
                    new_pos += np.array([np.cos(angle_to_other), np.sin(angle_to_other)]) * (
                                actor.radius + other_actor.radius)
