...
class PathfindingAlgorithm:
    ...
    def _reconstruct_path(self, came_from: Dict, current: Tuple[int, int], start: Tuple[int, int]) -> List[
        Tuple[int, int]]:
        path = []
...
def main():
    ...
    for actor in env.actors:
        actor.algorithm = algorithm_class(
            safety_weight=ACTOR_PROPERTIES.get(actor.actor_type, {}).get("safety_weight", 1.0))
    ...
    while running:
        ...
        for actor in env.actors:
            if actor.actor_type == "Patient":
                if actor.guiding is None:
                    # If not guided, look for a guide
                    ...
                    # If being guided, follow the guide
                    target_pos = actor.guiding.pos
            ...
            if path and len(path) > 1:
                ...
                if norm > 0:
                    ...
                    # Adjust speed based on actor type and guidance
                    move_distance = actor.speed * CELL_SIZE
                    if actor.actor_type == "Patient" and actor.guiding:
                        # Use guided speed for patients
                        guide_type = actor.guiding.actor_type.lower()
                        guided_speed = actor.guided_speeds.get(guide_type, 1.0)
                        move_distance = guided_speed * CELL_SIZE
    ...
    advanced_output_filename = f"advanced_evacuation_statistics_{selected_algorithm}.csv"
    env.export_metrics(output_filename)
    env.export_advanced_metrics(advanced_output_filename)
